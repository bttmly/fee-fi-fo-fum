...
class BeanstalkdClient extends EventEmitter {
  ...
  connect () {
    ...
    this._stream.on("data", chunk => {
      if (this._buffer == null) {
        ...
        const tmp = new Buffer(this._buffer.length + chunk.length);
        this._buffer.copy(tmp, 0);
        chunk.copy(tmp, this._buffer.length);
        this._buffer = tmp;
      }
  ...
  _tryHandlingResponse () {
    ...
    const oldest = this._handlers[0];
    const { handler, resolve, reject } = oldest;
